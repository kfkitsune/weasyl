...
def init_verify_tfa(userid, tfa_secret, tfa_response):
    """
    Verify that the user has successfully set-up 2FA in Google Authenticator (or similar), and
    ...
    Returns: A list of recovery codes as generated by `generate_recovery_codes()` if ``tfa_response``
    corresponds to ``tfa_secret``, otherwise ``None``.
    """
    totp = pyotp.TOTP(tfa_secret)
    # If the provided `tfa_response` matches the currently computed TOTP token, return a list of recovery codes.
    if totp.verify(tfa_response):
        return generate_recovery_codes()
    else:
        return None
...
def activate(userid, tfa_secret, tfa_response):
    """
    # TODO: Rename/Refactor to `store_totp_secret`? Since this can be used to store multiple secrets.
    ...
    if len(tfa_response) == LENGTH_TOTP_CODE:
        ...
        for row in tfa_secrets_encrypted:
            ...
            if totp_response_was_valid:
                ...
                return True
        # If we got here, then the TOTP token didn't correspond to a stored TOTP secret; return false
        return False
